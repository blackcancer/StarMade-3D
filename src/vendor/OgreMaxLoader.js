...
/** @param {number} loaded @param {number} total @returns {ProgressEvent} */
function makeProgressEvent(loaded, total) {
  // Event exists in browsers and supported Node versions, unlike ProgressEvent.
  return /** @type {ProgressEvent} */ (Object.assign(new Event('progress'), { loaded, total, lengthComputable: total > 0 }));
}
...
export class OgreMaxLoader extends THREE.Loader {
	 ...
	 * @param {(event:ProgressEvent)=>void}																			[onProgress]	- callback fired during loading, receives a Three.js-compatible progress event
	...
	load(url, onLoad = () => { }, onProgress = () => { }, onError = () => { }) {
		...
		return new Promise((resolve, reject) => {
			let failed = false;
			const manager = this.#internalManager = new THREE.LoadingManager();
			const fail = (err) => {
				if (failed) return;
				failed = true;
				...
				// Failing is not completion: outstanding requests must drain before
				// the same loader can be reused. Never synthesize itemEnd here.
				try { onError(err); } catch (callbackError) { err = callbackError; }
				reject(err);
			};
			...
			manager.onStart = (file, loaded, total) => {
				this.#logStart(file);
				console.log(`Started   : ${file}`);
				console.log(`Progress  : ${loaded}/${total}`);
			};

			manager.onProgress = (file, loaded, total) => {
				console.log(`Loading   : ${file}  (${loaded}/${total})`);

				// can be used to make progression bar
				try { onProgress(makeProgressEvent(loaded, total)); } catch (err) { fail(err); }
			};

			manager.onError = file => {
			...
			manager.onLoad = () => {
				...
				let result;
				try {
					if (!failed) result = this.#finalize();
				}
				catch (err) {
					fail(err);
				}
			...
			try { fileLoader.load(
				...
				progress => {
					try { onProgress(makeProgressEvent(progress.loaded, progress.total ?? 0)); } catch (err) { fail(err); }
				},
				() => {
					fail(new OgreMaxError("E_IO", `Cannot load ${url}`, { url }));
					manager.itemEnd(url);
				}
			); } catch (err) {
				fail(err);
	 ...
	 * return them before invoking completion callbacks, which may reuse the loader.
	 * @private
	 */
	#finalize() {
		...
		if (this.#objectRoot.mesh) {
			...
			if (this.#objectRoot.skeletonFile) {
				...
				// A multi-submesh file returns a Group. Keep the shared skeleton
				// under that root instead of reparenting its bones between meshes.
				mesh.add(...skel.bones.filter(bone => bone.parent === null));
		...
		if (this.#objectRoot.skeleton) {
			return this.#objectRoot.skeleton;
		}
		throw new OgreMaxError('E_FORMAT', 'Parser did not produce a scene, mesh or skeleton');
	}
	...
	/** Consume both dependency and attachment errors, and balance each request once. */
	#loadDependency(loader, url, attach) {
		const manager = this.#internalManager;
		manager.itemStart(url);
		loader.load(url).then(attach).then(
			() => manager.itemEnd(url),
			() => {
	...
	#parseMesh(XMLNode) {
		...
		if (skelLink) {
			...
			this.#loadDependency(intLoad,
				skelUrl,
				({ skeleton, animations }) => {

					// Bind after #finalize attaches the bones and updates world matrices.
	...
	#parseSkeleton(XMLNode) {
		...
		const bones = this.#parseBones(bonesNode);
		const animations = animationsNode ? this.#parseAnimations(animationsNode, bones) : [];
		const skeleton = new THREE.Skeleton(this.#parseBoneHierarchy(bonehierarchyNode, bones));
	 ...
	 * @returns {THREE.Object3D[]}				- array of parsed submeshes (Mesh, SkinnedMesh or Line)
	...
	/**
	 * Parse a submesh; only geometry with bone weights uses skeletal deformation.
	 * @private
	 * @param	{Element}																					XMLNode			- XML element `<submesh>`
	 * @param	{{vertices:THREE.Vector3[], normals:THREE.Vector3[], uvs:(THREE.Vector2|THREE.Vector3)[]}?}	shared			- optional shared geometry data (vertices, normals, uvs)
	 * @param	{number}																					materialSlot	- material slot index (for multi-materials)
	 * @returns {THREE.Mesh|THREE.SkinnedMesh|THREE.Line}																							- the resulting static, weighted or line geometry
	 */
	#parseSubmesh(XMLNode, shared = null, materialSlot = 0) {
		...
		const flatUvs = uvs.flatMap(u => typeof u === 'number' ? [u] : u.toArray());
		...
		if (flatUvs.length) {
			const uvSize = typeof uvs[0] === 'number' ? 1 : uvs[0].isVector3 ? 3 : 2;
			geom.setAttribute('uv', new THREE.Float32BufferAttribute(flatUvs, uvSize));
		}
		...
		if (indices.length) {
			const need32 = use32bitindexes || vertices.length > 65535;
		...
		// Static meshes must not enter SkinnedMesh bounds/raycast paths: they have
		// neither skin attributes nor a skeleton. Weighted meshes retain skinning.
		const mesh = skinIndex.length > 0
			? new THREE.SkinnedMesh(geom, material)
			: new THREE.Mesh(geom, material);
		mesh.name = XMLNode.getAttribute('name') || 'submesh';
		mesh.castShadow = true;
		mesh.receiveShadow = true;
		mesh.frustumCulled = false;
		mesh.userData.materialSlot = materialSlot;
		mesh.userData.operationType = opType;
		mesh.userData.usesharedvertices = usesShared;
		mesh.userData.use32bitindexes = use32bitindexes;
   ...
   */
	 ...
	 */
	#attrMatrix(XMLNode) {
		const position = this.#attrVector3(this.#querySelect(XMLNode, ':scope > position, :scope > translate'));
		const rotation = this.#attrQuaternion(this.#querySelect(XMLNode, ':scope > rotation, :scope > rotate'));
		const scaleNode = this.#querySelect(XMLNode, ':scope > scale');
	...
	/**
	 * Query a parser node; callers validate optional nodes before dispatch.
	 * @private
	 * @param	{Element}	XMLNode	- the parent node to query
	 * @param	{string}		sel		- the CSS selector to use
	 * @returns {Element|null}			- the first matching element or `null` if not found
	 */
	#querySelect(XMLNode, selector) {
		return XMLNode.querySelector(selector);
	}

	/**
	 * Query parser children after callers validate the parent node.
	 * @param	{Element}	XMLNode		- the parent node to query
	 * @param	{string}		selector	- the CSS selector to use
	 * @returns {Element[]}					- the list of matching elements or an empty array if not found
	 */
	#querySelectAll(XMLNode, selector) {
		return XMLNode.querySelectorAll(selector);
	}
...
export class DotMaterialLoader extends THREE.Loader {
	 ...
	 * @param	{function(ProgressEvent):void}			[onProgress]	- callback for Three.js-compatible progress events
	...
	load(url, onLoad = () => { }, onProgress = () => { }, onError = () => { }) {
		...
		return new Promise((resolve, reject) => {
			...
			fileLoader.load(
				...
				xhr => {
					try { onProgress(makeProgressEvent(xhr.loaded, xhr.total ?? 0)); } catch (error) { fail(error); }
				},
				() => fail(new DotMaterialError('E_IO', `Cannot load ${url}`, { url }))
			);
		});
	}
	...
	parse(text, texturePath = '') {
		 ...
		 * It supports StarMade/Open's diffuse, normal, and emissive texture classification.
		 * Emissive texture units are identified by `colour_op_ex add src_texture src_current`;
		 * `_nrm` textures are normal maps, matching the Java Ogre material parser.
		 * @private
		 * @param	{string}					matName	- the name of the material to create
		...
		function parsePassBlock(matName) {
			 ...
			 * StarMade-Open classifies texture units from the material script: `_nrm` means normal map,
			 * and `colour_op_ex add src_texture src_current` marks an emissive texture.
			 * @private
			...
			function handleTextureUnit() {
				...
				while (peek() !== '}') {
					...
					switch (t[0]) {
						case 'texture':
							...
							break;
						case 'colour_op_ex':
							emissive = t.slice(1).join(' ') === 'add src_texture src_current';
							break;
				...
				const texture = textureLoader.load(texturePath + textureName);

				texture.flipY = false; // OgreMax does not flip Y by default.
				if (emissive) {
					m.emissiveMap = texture;
					m.emissiveIntensity = 1;
					m.emissive.set(0xffffff);
				}
				else if (textureName.toLowerCase().includes('_nrm')) {
					m.normalMap = texture;
				}
				else {
					m.map = texture;
			 ...
			 * @param	{THREE.Color}	target	- the required material color property
