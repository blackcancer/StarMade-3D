...
try{
 ...
 let ready=false;for(let i=0;i<160;i++){ready=await ev('!!window.__CLOSE_AUDIT__&&!!window.__STARMADE_3D_READY__');if(ready)break;await delay(500)}if(!ready)throw Error('scene not ready: '+JSON.stringify(errors));
 ...
 const streaming=await ev('window.__STARMADE_STREAMING__');
 const shaderPreparation=await ev('window.__STARMADE_SHADER_PREPARATION__');
 if(!shaderPreparation || (shaderPreparation.enabled && shaderPreparation.phases.map(p=>p.name).join(',')!=='preview,final')) throw Error('Shader preparation incomplete');
 if(shaderPreparation.textureWarmupEnabled && (shaderPreparation.textureInitCallsMs.length!==11 || shaderPreparation.textureInitCallsMs.some(t=>!Number.isFinite(t)||t<0))) throw Error('Texture preparation incomplete');
 if(!streaming || streaming.status!=='complete' || streaming.segments!==4 || streaming.transferredBytes>=600*1024 || !(streaming.firstVisibleMs<streaming.totalMs)) throw Error('Streaming regression: '+JSON.stringify(streaming));
 await writeFile(out+'/result.json',JSON.stringify({ok:errors.length===0,errors,streaming,shaderPreparation,readyState:await ev('window.__STARMADE_3D_READY__')},null,2));
